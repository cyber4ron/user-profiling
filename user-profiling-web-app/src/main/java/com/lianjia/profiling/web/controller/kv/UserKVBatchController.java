...
import static com.lianjia.profiling.web.common.Constants.CUST_IDX;
import static com.lianjia.profiling.web.common.Constants.CUST_TYPE;
...
@RestController
public class UserKVBatchController {
    private static final Logger LOG = LoggerFactory.getLogger(UserKVBatchController.class.getName());

    private static final int QUERY_TIMEOUT_MS = 50000;

    private TransportClient client;

    public UserKVBatchController() throws UnknownHostException {
    ...
    private List<Map<String, Object>> offlineUserMultiGet(List<String> phones) {
        MultiGetResponse multiGetItemResponses = client.prepareMultiGet()
                .add(CUST_IDX, CUST_TYPE, phones)
    ...
    @RequestMapping(value = "/kv/batch/user/offline", method = RequestMethod.POST)
    @CrossOrigin
    public ResponseEntity<String> batchGetOfflineUser(@RequestBody Request.BatchKVRequest request) {
        try {
            if (!AccessManager.checkKV(request.token)) throw new IllegalAccessException();

            Map<String, Object> resp = new HashMap<>();
            List<?> list = offlineUserMultiGet(request.ids);
            resp.put("data", list);
