...
# 00 03 * * 1 cd /home/work/profiling-bin && bash backtrace_house_eval.sh >> /var/log/pf_backtrace_house_eval 2>&1
...
--class com.lianjia.profiling.stream.backtrace.HouseEvalLogBacktrace \
--master yarn --deploy-mode client \
--queue default \
--driver-memory 2G --executor-memory 1G \
--executor-cores 1  --num-executors 8 \
--conf spark.ui.port=8093 \
...
${JAR_DIR}/user-profiling-stream-${VERSION}.jar > ${LOG_DIR}/house_eval_`date '+%Y%m%d'`.log 2>&1 &

echo `date '+%Y%m%d'`", rc of command: $?"
